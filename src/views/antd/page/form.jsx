  ...
  render() {
    return (
      <div className="form">
        ...
        {data.map((item, i) => (
          <CheckboxItem key={i} onChange={() => changeHandler(item.value)}>
            {' '}
            {item.label}{' '}
          </CheckboxItem>
        ))}
        ...
        <List>
          ...
          <InputItem ref={(el) => (this.inputRef = el)} extra={<Icon type="right" />} clear defaultValue="10">
            请输入
          </InputItem>
          <List.Item
            extra={
          ...
          <List.Item
            extra={<Stepper style={{ width: '100%', minWidth: '50px' }} showNumber max={10} min={1} value={5} />}>
            Steper
          </List.Item>
        </List>
        <WhiteSpace />
        ...
        <Range
          handleStyle={[
            { height: '15px', width: '15px', marginTop: '-7px' },
            { height: '15px', width: '15px', marginTop: '-7px' },
          ]}
          min={0}
          max={20}
